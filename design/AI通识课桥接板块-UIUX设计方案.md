...
1. **厚边框**：卡片 `3px`，控件 `2.5px`，标签 `2px`。这是全站最强的视觉签名，比阴影更能定义"这是智用笔记"。
...
徽章这种形式，在用户的既有经验里表达的是"**本主体已被认证**"（蓝V、已认证商家、官方旗舰店）。把它贴在本站页面上，最直接的解读就是"智用笔记是官方认证的"——这正好是绝对不能发生的误解。徽章越精致，误解越深。
...
- **本站点评带署名和日期**：「智用笔记编辑部 · 8月更新」。敢署名 = 敢负责。
